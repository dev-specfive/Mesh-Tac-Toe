...
import com.geeksville.mesh.databinding.ActivityMainTabBinding
...
class MainTabActivity : AppCompatActivity(), Logging {
    ...
    private val inviteReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            bottomNavigationView.getOrCreateBadge(R.id.message_fragment).backgroundColor = ContextCompat.getColor(this@MainTabActivity, R.color.badge_color)
        }
    }

    fun resetBadge(){
        bottomNavigationView.getOrCreateBadge(R.id.message_fragment).backgroundColor = ContextCompat.getColor(this, R.color.zxing_transparent)
    }
