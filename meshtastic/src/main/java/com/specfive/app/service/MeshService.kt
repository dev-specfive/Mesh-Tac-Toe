...
import com.specfive.app.AdminProtos
import com.specfive.app.AppOnlyProtos
import com.specfive.app.ChannelProtos
import com.specfive.app.ConfigProtos
import com.specfive.app.CoroutineDispatchers
import com.specfive.app.DataPacket
import com.specfive.app.DeviceMetrics
import com.specfive.app.EnvironmentMetrics
import com.specfive.app.IMeshService
...
import com.specfive.app.MeshProtos.ToRadio
...
import com.specfive.app.Portnums
import com.specfive.app.Position
import com.specfive.app.R
import com.specfive.app.TelemetryProtos
import com.specfive.app.analytics.DataPair
...
import com.specfive.app.concurrent.handledLaunch
import com.specfive.app.config
import com.specfive.app.copy
import com.specfive.app.database.MeshLogRepository
import com.specfive.app.database.PacketRepository
import com.specfive.app.database.entity.MeshLog
import com.specfive.app.database.entity.Packet
import com.specfive.app.model.DeviceVersion
import com.specfive.app.repository.datastore.RadioConfigRepository
import com.specfive.app.repository.location.LocationRepository
import com.specfive.app.repository.network.MQTTRepository
import com.specfive.app.repository.radio.BluetoothInterface
import com.specfive.app.repository.radio.RadioInterfaceService
import com.specfive.app.repository.radio.RadioServiceConnectionState
import com.specfive.app.routeDiscovery
import com.specfive.app.util.anonymize
import com.specfive.app.util.exceptionReporter
import com.specfive.app.util.toOneLineString
import com.specfive.app.util.toPIIString
import com.specfive.app.util.toRemoteExceptions
import dagger.Lazy
...
class MeshService : Service(), Logging {
    ...
    companion object : Logging {
        ...
        fun createIntent() = Intent().setClassName(
            "com.specfive.meshtactoe",
            "com.specfive.app.service.MeshService"
        )
...
//            Portnums.PortNum.TEXT_MESSAGE_APP_VALUE -> dataPacket.text!!
            Portnums.PortNum.TEXT_MESSAGE_APP_VALUE -> getMessageContent(dataPacket)
    ...
    private fun getMessageContent(dataPacket: DataPacket):String{
        dataPacket.text?.apply {
            if (equals(InviteState.INVITE_SENT.title, true))
                return "You are invited by ${getSenderName(dataPacket)}"

            if (contains(InviteState.INVITE_ACCEPTED.title, true))
                return "Invitation accepted by ${getSenderName(dataPacket)}"
    ...
    private val appVersion get() = 30221
...
//                        playSound()
    ...
    private fun playSound() {
        ...
        val ringtone = RingtoneManager.getRingtone(baseContext, notificationSoundUri)
        ringtone.play()
    }

    private fun sendLocalBroadcastIfInviteAccepted(dataPacket: DataPacket) {
        val splitResult = dataPacket.text?.split("-")
        if (splitResult?.size == 3){
            if (dataPacket.text?.isNotBlank() == true && splitResult[0].equals(InviteState.INVITE_ACCEPTED.title, true)){
                val intent = Intent(InviteState.INVITE_ACCEPTED.title)
                intent.apply {
                    putExtra(EXTRA_ACCEPTED_CHANNEL_KEY, "${dataPacket.channel}${dataPacket.from}")
                    putExtra(EXTRA_SENDER_MSG_KEY, "${dataPacket.from}")
                    putExtra(EXTRA_INVITE_ACCEPTED_MSG_KEY, "${dataPacket.text}")
                }
    ...
    private fun sendLocalBroadcastIfInvited(dataPacket: DataPacket) {
        if (dataPacket.text?.equals(InviteState.INVITE_SENT.title, true) == true){
            val intent = Intent(InviteState.INVITE_SENT.title)
            LocalBroadcastManager.getInstance(baseContext).sendBroadcast(intent)
