...
import com.specfive.app.databinding.AdapterNodeLayoutBinding
import com.specfive.app.databinding.NodelistFragmentBinding
...
class UsersFragment : ScreenFragment("Users"), Logging {
    ...
    class ViewHolder(itemView: AdapterNodeLayoutBinding) : RecyclerView.ViewHolder(itemView.root) {
        ...
        val invite = itemView.sendInvite
    ...
    private val nodesAdapter = object : RecyclerView.Adapter<ViewHolder>() {
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            val showAdmin = position == 0 || model.adminChannelIndex > 0
            holder.invite.visibility = if (!showAdmin) View.VISIBLE else View.GONE
            holder.invite.setOnClickListener {
                val meshUser = nodes[position].user
                val channelId = nodes[position].channel
                if (!meshUser?.id.isNullOrBlank()){
                    meshUser?.id?.let { userid -> model.sendMessage(InviteState.INVITE_SENT.title, "$channelId$userid") }
                }
                else
